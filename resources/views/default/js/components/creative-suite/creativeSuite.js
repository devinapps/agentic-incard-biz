...
import { debounce, defer, difference, template, throttle } from 'lodash';
...
export default () => {
	return ({
		...
		guidelineOffset: 5,
		placeholderImage: 'https://placehold.co/600x400@2x?text=MagicAI',
		templatesList: [],
		...
		selectionTransformerEvents() {
			...
			this.selectionTransformer.on('transform', event => {
		...
		createNode(options = { type: null, attrs: {} }) {
			...
			if ( options.type === 'Image' ) {
				this.handleImageUpload(node, attrs.fillSource || this.placeholderImage);
			}
		...
		handleTextEdit(node) {
			...
			node.on('dblclick.editText dbltap.editText', () => {
				...
				this.editingTextarea.style.width = '9999px';
				this.editingTextarea.style.fontFamily = node.fontFamily();
				this.editingTextarea.style.lineHeight = type === 'Text' ? node.lineHeight().toString() : '1.2';
				...
				const handleOutsideClick = event => {
					...
					if (event.target !== this.editingTextarea) {
						node.text(this.editingTextarea.value);
				...
				defer(() => {
					...
					window.addEventListener('tap', handleOutsideClick);
				});
			});
		...
		positionEditingTextArea() {
			...
			this.editingTextarea.style.fontSize = (this.editingTextNode.fontSize() * zoom) + 'px';
			this.editingTextarea.style.letterSpacing = (this.editingTextNode.letterSpacing() * zoom) + 'px';

			if (rotation && type !== 'TextPath') {
		...
		async handleImageUpload(node, imageSource) {
			if ( imageSource ) {
				return fetch(imageSource)
					...
					.then(blob => {
						...
						this.uploadImageAndSetFill({ file, node });
					})
					.catch(error => {
			...
			fileInput.addEventListener('change', async event => {
				await this.uploadImageAndSetFill({ event, node });
		...
		async uploadImageAndSetFill({ event, file, node, setSize = true }) {
			if ( event && !file ) {
				const input = event.target;
				file = input.files[0];
			}
		...
		getFitToScreenZoom() {
			const containerWidth = this.$refs.editorCanvasContainer.clientWidth;
			const containerHeight = this.$refs.editorCanvasContainer.clientHeight;
			const stageWidth = this.stage.width();
			const stageHeight = this.stage.height();
			const paddingX = 80;
			const paddingY = 80;
			...
			const zoomX = (availableWidth / stageWidth) * 100;
			const zoomY = (availableHeight / stageHeight) * 100;
			const calculatedZoom = Math.min(zoomX, zoomY);
			const minZoom = this.getMinZoom();
			const maxZoom = this.getMaxZoom();
			const fitZoom = Math.max(minZoom, Math.min(calculatedZoom, maxZoom));
		...
		setZoomLevel(level) {
			...
			this.reachedMinZoom = this.zoomLevel === minZoom;
		...
		positionTooltip() {
			...
			const x = contentRect.left + (rect.x * zoom) + (rect.width * zoom / 2);
			const y = contentRect.top + (rect.y * zoom) - 40;
		...
		initWheelZoom() {
			...
			const handleWheel = e => {
				...
				if (e.metaKey || e.ctrlKey) {
					...
					const delta = direction * zoomAmount;
					const minZoom = this.getMinZoom();
					const maxZoom = this.getMaxZoom();

					const newZoomLevel = Math.max(minZoom, Math.min(
						maxZoom,
					...
					const mouseOffsetX = (mouseX - viewportCenterX) / oldZoom;
					const mouseOffsetY = (mouseY - viewportCenterY) / oldZoom;

					this.zoomOffsetX -= mouseOffsetX * (zoomFactor - 1);
					this.zoomOffsetY -= mouseOffsetY * (zoomFactor - 1);
				} else {
					this.zoomOffsetX -= e.deltaX / (this.zoomLevel / 15);
					this.zoomOffsetY -= e.deltaY / (this.zoomLevel / 15);
				}
			};
			...
			const handleKeyDown = e => {
				...
				if (e.code === 'Space' && !e.repeat && !this.isSpacePressed && this.nodes.length) {
			...
			const handleMouseDown = e => {
				...
				if (this.isSpacePressed && e.button === 0 && this.nodes.length) {
					this.isPanning = true;
			...
			this.selectionTransformer.on('dragmove', this.positionTooltip);
		...
		initNodesSnap() {
			...
			this.selectionTransformer.on('dragmove', () => {
				...
				this.selectedNodes.forEach(node => {
					...
					const xDiff = nodeAbsPos.x - transformerAbsPos.x;
					const yDiff = nodeAbsPos.y - transformerAbsPos.y;

					// now force object position
					guides.forEach(lg => {
						switch (lg.orientation) {
							case 'V': {
								nodeAbsPos.x = lg.lineGuide + lg.offset + xDiff;
								break;
							}
							case 'H': {
								nodeAbsPos.y = lg.lineGuide + lg.offset + yDiff;
								break;
							}
						}
					});

					node.absolutePosition(nodeAbsPos);
				});
			});
		...
		resetCanvas() {
			if (
				this.nodes.length &&
				confirm(magicai_localize['are_you_sure_'])
			) {
				this.resetHistory();
				this.removeNodes({ nodes: 'all' });
				this.fitToScreen();
			}
		},
			...
			if (
				...
				if (importData.stage) {
					this.handleStageResize({
						width: importData.stage.width,
						height: importData.stage.height
					});
				}
		...
		loadTemplate(templateId) {
			...
			this.importData(templateData.data);
		},
		...
		async generateUploadingImage(imageUrl) {
