...
trait HasApiKeys
{
    ...
    public static function setCreatifyAIKey(): array
    {
        ...
        if (! app()->runningInConsole()) {
            $id = setting('creatify_api_id');
            $key = setting('creatify_api_key');
        }
        config([
            'ai-influencer.creatify_api_id'  => $id,
            'ai-influencer.creatify_api_key' => $key,
        ]);

        return [
            'creatify_api_id'  => config('ai-influencer.creatify_api_id', 'empty'),
            'creatify_api_key' => config('ai-influencer.creatify_api_key', 'empty'),
        ];
    }
    ...
    public static function setTopviewKey(): array
    {
        ...
        config([
            'ai-influencer.topview_api_id'  => $id,
            'ai-influencer.topview_api_key' => $key,
        ]);

        return [
            'topview_api_id'  => config('ai-influencer.topview_api_id', 'empty'),
            'topview_api_key' => config('ai-influencer.topview_api_key', 'empty'),
        ];
    }
