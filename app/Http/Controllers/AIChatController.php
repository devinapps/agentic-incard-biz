...
class AIChatController extends Controller
{
    ...
    public function uploadDoc(Request $request, $chat_id, $type)
    {
        ...
        $detectedMimeType = $doc->getMimeType();
        if (! in_array($detectedMimeType, $allowedMimeTypes)) {
            throw new Exception('File type not allowed');
        }

        $originalName = $doc->getClientOriginalName();
        $realExtension = strtolower($doc->getClientOriginalExtension());
        ...
        if (substr_count($originalName, '.') > 1) {
            throw new Exception(trans('There cannot be more than one dot in the file name.'));
        }
        ...
        $expectedExtensions = $mimeExtensionMap[$detectedMimeType];
        if (is_array($expectedExtensions)) {
            if (! in_array($realExtension, $expectedExtensions)) {
                throw new Exception('MIME type and extension mismatch');
            }
        } else {
            if ($realExtension !== $expectedExtensions) {
                throw new Exception('MIME type and extension mismatch');
            }
        }
