...
<style>
    ...
    .lqd-ext-chatbot-voice.fullscreen {
        width: 98vw;
        height: 100vh;
        right: 0;
        bottom: 0;
    ...
    .lqd-ext-chatbot-voice.fullscreen .icon img {
        border-radius: 50%;
        height: 160px;
        width: 160px;
    }

    .lqd-ext-chatbot-voice.fullscreen .icon {
        order: 1;
        width: 50%;
        height: auto;
    ...
    .lqd-ext-chatbot-voice.fullscreen .lqd-ext-chatbot-voice-trigger {
        order: 2;
        /* bottom: 65px; */
        position: relative;
    }

    .lqd-ext-chatbot-voice-trigger p {
        text-align: center;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    const startBtn = document.getElementById('lqd-ext-chatbot-voice-start-btn');
    const endBtn = document.getElementById('lqd-ext-chatbot-voice-end-btn');
    
    if (startBtn) {
        startBtn.addEventListener('click', function() {
            const wrapper = document.querySelector('.lqd-ext-chatbot-voice');
			if (wrapper) {
            ...
            parent.postMessage({
                type: 'lqd-ext-voice-chatbot-start-btn-clicked',
                message: ""
            }, '*');
        });
    }
    ...
    if (endBtn) {
        ...
        // Tạo MutationObserver để tracking thay đổi style
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    const currentDisplay = window.getComputedStyle(endBtn).display;
                    
                    if (currentDisplay === 'flex') {
                        console.log('End button is now visible');
                        // Thực hiện action khi button hiển thị
                        handleEndButtonVisible();
                    } else if (currentDisplay === 'none') {
                        console.log('End button is now hidden');
                        // Thực hiện action khi button ẩn
                        handleEndButtonHidden();
                    }
                }
            });
        });
        ...
        // Bắt đầu observe
        observer.observe(endBtn, {
            attributes: true,
            attributeFilter: ['style']
        });

        // Xử lý Click event cho nút End
        endBtn.addEventListener('click', function() {
            ...
            // Send message to parent window
            parent.postMessage({
                type: 'lqd-ext-voice-chatbot-end-btn-clicked',
                message: ""
            }, '*');
        });
    }

    function handleEndButtonVisible() {
        // Logic khi button hiển thị
        console.log('End button became visible - do something');
    }

    function handleEndButtonHidden() {
        // Logic khi button ẩn
        console.log('End button became hidden - do something');
        const wrapper = document.querySelector('.lqd-ext-chatbot-voice');
        if (wrapper) {
            wrapper.classList.remove('fullscreen');
        }

        // Gửi thông điệp đến parent window - để thông báo nút End đã được ẩn
        parent.postMessage({
            type: 'lqd-ext-voice-chatbot-end-btn-clicked',
            message: ""
        }, '*');
