...
class OpenAIGenerator extends Generator
{
    ...
    public function generate(): string
    {
        ...
        top_p=0.3,                  # Limits the sampling pool to the most likely tokens
        frequency_penalty=0.0,      # No penalty on repeating tokens (useful for factual repetition)
        presence_penalty=0.0,       # No penalty on introducing required factual tokens
        max_tokens=300,             # Adjust as needed to fully cover expected responses
        stop=None                   # Use stop sequences if necessary to limit output
        */
        $body = [
            'temperature' => 0.0,
            'top_p'       => 1.0,
    ...
    public function tools(): array
    {
        return [
            'tools' => [
                ...
                // New tool definition starts here
                [
                    'type'     => 'function',
                    'function' => [
                        'name'        => 'store_lead_data',
                        'description' => 'tool này dùng để lưu thông tin Name, Email, Phone của Lead khi chatbot nhận được thông tin Lead lúc chat',
                        'parameters'  => [
                            'type'       => 'object',
                            'properties' => [
                                'name' => [
                                    'type'        => 'string',
                                    'description' => 'Tên của Lead.', // Name of the Lead.
                                ],
                                'email' => [
                                    'type'        => 'string',
                                    'description' => 'Địa chỉ email của Lead.', // Email address of the Lead.
                                ],
                                'phone' => [
                                    'type'        => 'string',
                                    'description' => 'Số điện thoại của Lead.', // Phone number of the Lead.
                                ],
                            ],
                            'required' => [
                                'name',
