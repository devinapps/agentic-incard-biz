...
class SocialMediaCampaignController extends Controller
{
    ...
    private function hasPersonalizedContent(Request $request)
    {
        ...
        $systemPrompt = "You are a professional social media content creator with a deep understanding of human psychology, brand storytelling, and audience engagement. For {$platform}, create content that feels authentic, conversational, and emotionally resonant — so natural that it’s indistinguishable from human writing. Always keep it strictly within {$limit} characters, including emojis and punctuation. Use emojis only where they enhance meaning, never as decoration. Balance creativity with clarity, maintain the brand’s unique tone, and ensure the content encourages meaningful interaction.";

        $userPrompt = <<<EOT
            Create a {$platform}-style post using the following input: "{$content}".
            The post should be {$tone} and designed to spark genuine engagement, while feeling like it was written by a real human, not AI.
            Strictly keep it within {$limit} characters, including emojis and punctuation.
            Use emojis only where they add meaning or emotional connection — avoid overuse.
            Incorporate relevant hashtags and a clear, compelling call to action.
            Ensure the post reflects the brand’s {$brandTone}, {$brandDesc}, and {$productDesc}, while staying consistent with its personality.
            The content must connect with the target audience for the {$campaignName} campaign, using relatable language, subtle storytelling, and a natural flow that encourages response.
            EOT;

        Log::info('>>> openai_default_model1 : ' . Helper::setting('openai_default_model'));
    ...
    private function hasNotPersonalizedContent(Request $request): CreateResponse
    {
        ...
        $userPrompt = <<<EOT
            Create a {$platform}-style post using the following input: "{$userContent}".
            The post should be {$tone} and designed to spark genuine engagement, while feeling like it was written by a real human, not AI.
            Strictly keep it within {$limit} characters, including emojis and punctuation.
            Use emojis only where they add meaning or emotional connection — avoid overuse.
            Incorporate relevant hashtags and a clear, compelling call to action.
            Ensure the post reflects the brand’s {$tone},while staying consistent with its personality.
            The content must be using relatable language, subtle storytelling, and a natural flow that encourages response.
            EOT;

        Log::info('>>> openai_default_model2: ' . Helper::setting('openai_default_model'));
