...
class ChatbotApplicationController extends Controller
{
    ...
    public function storeMessage(ChatbotHistoryStoreRequest $request, Chatbot $chatbot, string $sessionId, ChatbotConversation $chatbotConversation)
    {
        if ($chatbotConversation->getAttribute('chatbot_id') !== $chatbot->getAttribute('id')) {
            abort(404);
        }
        ...
        Log::info('>>> debug');
        Log::info($chatbot->hook_url && !$chatbot->notification_email);

        //Auto call to N8N Flow to notify the user
        if($chatbot->notification_email) {
            ...
            Http::timeout(120)->post('https://auto-agent.incard.biz/webhook/5c7ef248-ec2b-4473-b2aa-396a196318df', [
                'prompt' => $request->prompt,
                'sessionId' => $chatbotConversation->id,
                'notification_email' => $chatbot->notification_email
            ]);
        }

        if($chatbot->hook_url) {
            $userMessage = $this->insertMessage($chatbotConversation, $request->validated('prompt'), 'user', '<from_auto-agent>');

            $response = Http::timeout(120)->post($chatbot->hook_url, [
                'prompt' => $request->prompt,
                'sessionId' => $chatbotConversation->id,
                'notification_email' => $chatbot->notification_email
            ]);
            ...
            Log::info($chatbot->hook_url);
            Log::info($response);

            $hookMsg = '';
            if(isset($data['output'])) {
                $hookMsg = $data['output'];
                $message = $this->insertMessage($chatbotConversation, $data['output'], 'assistant', '<from_auto-agent>');
            }
            else if(isset($data[0]['output'])) {
                $hookMsg = $data[0]['output'];
                $message = $this->insertMessage($chatbotConversation, $data[0]['output'], 'assistant', '<from_auto-agent>');
            }
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    "role" => "assistant",
                    'message' => $hookMsg,
                    "created_at" => "2025-04-14T04:28:14.000000Z",
                    "read_at" => "2025-04-14T04:28:14.449040Z"
                ]
            ]);
        }
        else {
            ...
            // //Auto call to N8N Flow to notify the user
            // if($chatbot->hook_url) {
            //     Http::timeout(120)->post($chatbot->hook_url, [
            //         'prompt' => $request->prompt,
            //         'sessionId' => $chatbotConversation->id,
            //         'notification_email' => $chatbot->notification_email
            //     ]);
