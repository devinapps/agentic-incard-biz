	...
	<div class="flex flex-col gap-7 pt-9">
		<div>
			<x-forms.input
				...
				label="{{ __('Chatbot hook URL') }}"
				placeholder=""
				name="hook_url"
				size="lg"
				x-model="activeChatbot.hook_url"
		...
		<div>
			<x-forms.input
				...
				size="lg"
				x-model="activeChatbot.notification_email"
				@input.throttle.250ms="externalChatbot && externalChatbot.toggleWindowState('open')"
			/>
		</div>
		<div>
			<x-forms.input
				...
				label="{{ __('Activate Facebook messenger response') }}"
				name="is_facbook_msg_active"
				size="lg"
				type="checkbox"
				switcher
				x-model.boolean="activeChatbot.is_facbook_msg_active"
			/>
		</div>
		<div>
			<x-forms.input
